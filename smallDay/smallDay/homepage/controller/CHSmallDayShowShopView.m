...
@interface CHSmallDayShowShopView() <UIWebViewDelegate,UIScrollViewDelegate>
...
@property (nonatomic, strong) CHSmallDayLabelCollView *labelBgView;
@property (nonatomic, strong) NSString *hhhh;
@property (nonatomic, strong) NSArray *userArray;
@property (nonatomic, strong) CHSmalldayShowAllView *shopUser;
@end
@implementation CHSmallDayShowShopView
...
- (void)viewDidLoad
{
    ...
    web.delegate = self;
    self.src.contentInset = UIEdgeInsetsMake(400, 0, 100, 0);
    self.src.contentOffset = CGPointMake(0, -400);
    ...
    CHweakSelf;
    if (!_headerImage) {
        _headerImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, -400, CHScreenW, 200)];
        //[_headerImage sd_setImageWithURL:[NSURL URLWithString:_hhhh]];
    ...
    if (!_labelBgView) {
        _labelBgView  = [[CHSmallDayLabelCollView alloc]initWithFrame:CGRectMake(0, -200, CHScreenW, 200)];
        _labelBgView.backgroundColor = [UIColor whiteColor];
        [self.web.scrollView addSubview:_labelBgView];
    }
    ...
    if (!_backButton) {
        ...
        [_backButton addTarget:self action:@selector(popVc) forControlEvents:UIControlEventTouchUpInside];
    ...
    [[CHNetWorking sharedManager]getWithUrlString:@"http://apiv2.xiaorizi.me/shop/detail/?all=1&app_token=B3868EDE00863D2E&channel=iTunes&shop_id=200183&token_time=1517638120&user_id=205143&uuid=5232B239-4C44-4739-B706-3701EADA0919&version=7.0.4" withParams:nil andBlock:^(id data, NSError *error) {
        ...
        NSArray    *useraArray = [ChsmalldayshowusermodelModel mj_objectArrayWithKeyValuesArray:data[@"useravatar"]];
        if (useraArray) {
            self.shopUser.userArray = useraArray;
        }
        //[self.headerImage sd_setImageWithURL:[NSURL URLWithString:shop.img]];
        
                       if (shop) {
                           
                           self.labelBgView.shop = shop;
                           
                           _hhhh = shop.img;
                           //[CHSmallDayLoading hideInView:self.view];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if (!_shopUser) {
        _shopUser = [[CHSmalldayShowAllView alloc]initWithFrame:view.bounds];
        [view addSubview:_shopUser];
    }
    NSLog(@"???????");
    //[CHSmallDayLoading hideInView:self.view];
...
- (void)dealloc{
    [self.web.scrollView removeObserver:self forKeyPath:@"contentOffset"];
}
@end
